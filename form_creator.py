...
def create_form(goal, problem, form_length, allowed_types, solution_task):
    ...
    create_form_user_prompt.replace("<GOAL>", goal)
    create_form_user_prompt.replace("<PROBLEM>", problem)
    create_form_user_prompt.replace("<FORM_LENGTH>", form_length)
    create_form_user_prompt.replace("<ALLOWED_TYPES>", str(allowed_types))
    create_form_user_prompt.replace("<SOLUTION_TASK>", solution_task)
...
def submit_form(goal, problem, solution_task, responses):
    ...
    submit_form_user_prompt.replace("<GOAL>", goal)
    submit_form_user_prompt.replace("<PROBLEM>", problem)
    submit_form_user_prompt.replace("<SOLUTION_TASK>", solution_task)
    submit_form_user_prompt.replace("<RESPONSES>", str(list(responses.items())))
